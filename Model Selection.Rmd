...
Exercise in the back of the book

Applied- Problem 9  (pg. 263)
...
Sample training data set - 500/777
  The test set will be the remainder [-train]
```{r}
train<-sample(seq(777),size=500,replace=FALSE)
```

Fit a linear model on training and report test
```{r}
model.fwd=regsubsets(Apps~.,data=College[train,],nvmax=19,method="forward")
summary(model.fwd)
plot(model.fwd,scale="Cp")

val.errors=rep(NA,17)
x.test=model.matrix(Apps~.,data=College[-train,])
for(i in 1:17){
  coefi=coef(model.fwd,id=i)
  pred=x.test[,names(coefi)]%*%coefi
  val.errors[i]=mean((College$Apps[-train]-pred)^2)
}

plot(val.errors)
val.errors[which.min(val.errors)]
sqrt(val.errors[which.min(val.errors)])

```

The minimum test error is the full model, so that will be linear model predicted - the average prediction error 1014.967

(c) Use ridge regression to predict
```{r}
